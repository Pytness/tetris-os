...
void isr_install(size_t i, void (*handler)(struct Registers *)) { handlers[i] = handler; }
...
static void exception_handler(struct Registers * regs) { panic(exceptions[regs->int_no]); }
