...
#define TTM_OFFSET_X(_t)                                  \
	MIN(_t & 0x000F ? LOBIT((_t >> 0) & 0xF) : 3,     \
		...
		MIN(_t & 0x0F00 ? LOBIT((_t >> 8) & 0xF) : 3, _t & 0xF000 ? LOBIT((_t >> 12) & 0xF) : 3)))

#define TTM_WIDTH(_t)                                                                                    \
	1 +                                                                                              \
		MAX(HIBIT((_t >> 0) & 0xF),                                                              \
		    MAX(HIBIT((_t >> 4) & 0xF), MAX(HIBIT((_t >> 8) & 0xF), HIBIT((_t >> 12) & 0xF)))) - \
		TTM_OFFSET_X(_t)
...
static const struct Tetromino TETROMINOS[NUM_TETROMINOS] = {
	{ // line
	  .color     = CYAN,
	  .rotations = { 0x00F0, 0x2222, 0x0F00, 0x4444 } },
	{ // left L
	  .color     = BLUE,
	  .rotations = { 0x8E00, 0x6440, 0x0E20, 0x44C0 } },
	{ // right L
	  .color     = ORANGE,
	  .rotations = { 0x2E00, 0x4460, 0x0E80, 0xC440 } },
	{ // cube
	  .color     = YELLOW,
	  .rotations = { 0xCC00, 0xCC00, 0xCC00, 0xCC00 } },
	{ // right skew
	  .color     = GREEN,
	  .rotations = { 0x6C00, 0x4620, 0x06C0, 0x8C40 } },
	{ // left skew
	  .color     = RED,
	  .rotations = { 0xC600, 0x2640, 0x0C60, 0x4C80 } },
	{ // T
	  .color     = PURPLE,
	  .rotations = { 0x4E00, 0x4640, 0x0E40, 0x4C40 } }
};

#define NUM_LEVELS 30

// from listfist.com/list-of-tetris-levels-by-speed-nes-ntsc-vs-pal
static u8 FRAMES_PER_STEP[NUM_LEVELS] = { 48, 43, 38, 33, 28, 23, 18, 13, 8, 6, 5, 5, 5, 4, 4,
					  4, 3, 3, 3, 2, 2, 2, 2, 2, 2, 2, 2, 2, 1 };

static u32 LINE_MULTIPLIERS[4] = { 40, 100, 300, 1200 };
...
static void done() {
	...
	TTM_FOREACH(x, y, xx, yy, state.curr.x, state.curr.y) {
		if (IN_BOARD(xx, yy) && TTM_BLOCK(state.curr.ttm->rotations[state.curr.r], x, y)) {
...
static bool try_modify(const struct Tetromino * ttm, u16 tc, i32 xc, i32 yc, u16 tn, i32 xn, i32 yn) {
	u8 board[BOARD_HEIGHT][BOARD_WIDTH];
	...
	TTM_FOREACH(x, y, xx, yy, xn, yn) {
		if (yy < 0) {
			...
			continue;
		} else if (!IN_BOARD(xx, yy) || state.board[yy][xx] != NONE || xx < 0 || xx > (BOARD_WIDTH - 1)) {
			goto fail;
		}
...
static bool spawn() {
	...
	if (!try_modify(state.curr.ttm, 0, 0, 0, state.curr.ttm->rotations[state.curr.r], state.curr.x, state.curr.y)) {
		return false;
	}
...
static bool move(i32 dx, i32 dy) {
	if (try_modify(state.curr.ttm, state.curr.ttm->rotations[state.curr.r], state.curr.x, state.curr.y,
		       state.curr.ttm->rotations[state.curr.r], state.curr.x + dx, state.curr.y + dy)) {
		state.curr.x += dx;
		state.curr.y += dy;
		return true;
	}
...
static bool rotate(bool right) {
	...
	if (try_modify(state.curr.ttm, state.curr.ttm->rotations[state.curr.r], state.curr.x, state.curr.y,
		       state.curr.ttm->rotations[r], state.curr.x, state.curr.y)) {
		state.curr.r = r;
		return true;
	}
...
static void render_board() {
	for (size_t y = 0; y < BOARD_HEIGHT; y++) {
		for (size_t x = 0; x < BOARD_WIDTH; x++) {
			...
			size_t xs = BOARD_X + (x * TILE_SIZE), ys = BOARD_Y + (y * TILE_SIZE);
...
#define RENDER_STAT(_title, _value, _color, _x, _y, _w)                                          \
	do {                                                                                     \
		char buf[32];                                                                    \
		itoa((_value), buf, 32);                                                         \
	...
	for (size_t j = 0; j < TTM_SIZE; j++) {
		for (size_t i = 0; i < TTM_SIZE; i++) {
			...
			if (TTM_BLOCK(tiles, i, j)) {
				render_tile(state.next->color, X_OFFSET_LEFT + ((i - TTM_OFFSET_X(tiles)) * TILE_SIZE),
...
static void render_game_over() {
	...
	screen_fill(COLOR(4, 4, 2), (SCREEN_WIDTH - w) / 2, (SCREEN_HEIGHT - h) / 2, w, h);

	screen_fill(COLOR(2, 2, 1), (SCREEN_WIDTH - (w - 8)) / 2, (SCREEN_HEIGHT - (h - 8)) / 2, w - 8, h - 8);

	font_str_doubled("GAME OVER", (SCREEN_WIDTH - font_width("GAME OVER")) / 2, (SCREEN_HEIGHT / 2) - TILE_SIZE,
			 (state.frames / 5) % 2 == 0 ? COLOR(6, 2, 1) : COLOR(7, 4, 2));

	char buf_score[64];
	itoa(state.score, buf_score, 64);

	font_str_doubled("SCORE:", (SCREEN_WIDTH - font_width("SCORE:")) / 2, (SCREEN_HEIGHT / 2), COLOR(6, 6, 0));

	font_str_doubled(buf_score, (SCREEN_WIDTH - font_width(buf_score)) / 2, (SCREEN_HEIGHT / 2) + TILE_SIZE,
			 COLOR(7, 7, 3));
}
...
static void update() {
	...
	for (size_t y = 0; y < BOARD_HEIGHT; y++) {
		...
		if (destroy) {
			if (y != 0) {
				memmove(&state.board[1], &state.board[0], sizeof(state.board[0]) * y);
			}
	...
	const bool control_states[NUM_CONTROLS] = {
		keyboard_char('a'), keyboard_char('d'),
		keyboard_char('r'), keyboard_key(KEY_LEFT),
		keyboard_key(KEY_RIGHT), keyboard_key(KEY_DOWN),
		keyboard_char(' ')
	};
	...
	if (state.controls.rotate_left.pressed) {
		rotate(false);
	} else if (state.controls.rotate_right.pressed || state.controls.rotate.pressed) {
		rotate(true);
	}

	if (state.controls.left.down && (state.frames - state.controls.left.pressed_frames) % 2 == 0) {
		move(-1, 0);
	} else if (state.controls.right.down && (state.frames - state.controls.right.pressed_frames) % 2 == 0) {
		move(1, 0);
	} else if (state.controls.down.down && (state.frames - state.controls.down.pressed_frames) % 2 == 0) {
		if (!move(0, 1)) {
			done();
		}
...
void render_menu() {
	...
	// render logo
	size_t logo_width = strlen(LOGO[0]), logo_x = (SCREEN_WIDTH - (logo_width * TILE_SIZE)) / 2,
	       logo_y = TILE_SIZE * 3;
	...
	for (size_t y = 0; y < LOGO_HEIGHT; y++) {
		for (size_t x = 0; x < logo_width; x++) {
			...
			render_tile(GREEN + ((((state.frames / 10) + (6 - (c - 'A'))) / 6) % 8),
				    logo_x + (x * TILE_SIZE), logo_y + (y * TILE_SIZE));
		}
	}

	const char * play = "PRESS ENTER TO PLAY";
	font_str_doubled(play, (SCREEN_WIDTH - font_width(play)) / 2, logo_y + ((LOGO_HEIGHT + 6) * TILE_SIZE),
			 (state.frames / 6) % 2 == 0 ? COLOR(6, 6, 2) : COLOR(7, 7, 3));
}
